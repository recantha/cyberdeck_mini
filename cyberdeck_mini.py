...
db = database()
city_name = "Potton"
add_locations("Potton,Bedfordshire,UTC,52°1271'N,0°2152'W", db)
...
font_xl = ImageFont.truetype(UserFont, 18)
...
def display_ip_address():
    global img
    global ip_address
    procres = subprocess.check_output(['hostname', '-I'])
    ip_address = procres.decode('utf-8').split('\n')[0]
    img = overlay_text(img, (WIDTH-margin, 0+margin), ip_address, font_lg, align_right=True)
...
def full_display():
    global img, cpu_temps, min_temp, max_temp
...
def get_memory_usage():
    virtual_memory = "{}%".format(psutil.virtual_memory()[2])
    swap_memory = "{}%".format(psutil.swap_memory()[3])
    readings = (virtual_memory, swap_memory)
...
def next_mode():
    global mode, initial_mode, max_mode
    ...
    print("Eventual mode " + str(mode))

def statistics():
    cpu_usage = "CPU usage: {}".format(get_cpu_usage())
    (virtual_memory, swap_memory) = get_memory_usage()
    vram_usage = "Virtual: {}".format(virtual_memory)
    sram_usage = "Swap: {}".format(swap_memory)
    disk_usage = "Disk: {}".format(get_disk_usage())

    img = Image.new('RGBA', (WIDTH, HEIGHT), color="red")
    img = overlay_text(img, (0+margin, 0+margin), cpu_usage, font_xl, align_right=False)
    img = overlay_text(img, (0+margin, 20+margin), vram_usage, font_xl)
    img = overlay_text(img, (0+margin, 40+margin), sram_usage, font_xl)
    img = overlay_text(img, (0+margin, 60+margin), disk_usage, font_xl)
